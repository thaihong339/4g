...
# Sets up or updates KernelSU environment
setup_kernelsu() {
    ...
    # Force custom version string shown in SukiSU Manager
    CUSTOM_KSU_VERSION="v3.1.7-${KSU_VERSION:-custom}@susfs-main"
    cat <<EOF > "$GKI_ROOT/KernelSU/scripts/gen_version.sh"
#!/bin/sh
echo "$CUSTOM_KSU_VERSION"
EOF
    chmod +x "$GKI_ROOT/KernelSU/scripts/gen_version.sh"
    echo "[+] Overridden KernelSU version string to: $CUSTOM_KSU_VERSION"
