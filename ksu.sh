...
# Inject custom KSU version
KSU_VERSION=$(expr $(/usr/bin/git rev-list --count main) "+" 10700)
export KSU_VERSION=$KSU_VERSION
sed -i "s/DKSU_VERSION=.*/DKSU_VERSION=${KSU_VERSION}/" kernel/Makefile || error "Failed to modify KernelSU version"
echo "echo \"v3.1.7-${KSU_VERSION}@susfs-main\"" > scripts/get_ksu_version.sh
chmod +x scripts/get_ksu_version.sh
