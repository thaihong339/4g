...
export CCACHE_COMPILERCHECK="%compiler% -dumpmachine; %compiler% -dumpversion" export CCACHE_NOHASHDIR="true" export CCACHE_HARDLINK="true" export CCACHE_DIR="$HOME/.ccache_${DEVICE_NAME}" export CCACHE_MAXSIZE="8G"

CCACHE_INIT_FLAG="$CCACHE_DIR/.ccache_initialized"

if command -v ccache >/dev/null 2>&1; then if [ ! -f "$CCACHE_INIT_FLAG" ]; then info "Initializing ccache for ${DEVICE_NAME} for the first time..." mkdir -p "$CCACHE_DIR" || error "Failed to create ccache directory" ccache -M "$CCACHE_MAXSIZE" touch "$CCACHE_INIT_FLAG" else info "ccache (${DEVICE_NAME}) already initialized, skipping..." fi else info "ccache not installed, skipping initialization" fi

WORKSPACE="$GITHUB_WORKSPACE/kernel_${DEVICE_NAME}" mkdir -p "$WORKSPACE" || error "Failed to create working directory" cd "$WORKSPACE" || error "Failed to enter working directory"

if ! command -v repo >/dev/null 2>&1; then info "Installing repo tool..." curl -fsSL https://storage.googleapis.com/git-repo-downloads/repo > ~/repo || error "Failed to download repo" chmod a+x ~/repo mv ~/repo /usr/local/bin/repo || error "Failed to install repo" else info "Repo tool already installed, skipping" fi

KERNEL_WORKSPACE="$WORKSPACE/kernel_workspace" mkdir -p "$KERNEL_WORKSPACE" || error "Failed to create kernel_workspace directory" cd "$KERNEL_WORKSPACE" || error "Failed to enter kernel_workspace directory"
...
cd "$WORKSPACE" || error "Failed to return to workspace" git clone -q https://github.com/thaihong339/AnyKernel3.git --depth=1 || info "AnyKernel3 already exists" rm -rf ./AnyKernel3/.git ./AnyKernel3/push.sh cp "$KERNEL_WORKSPACE/kernel_platform/common/out/arch/arm64/boot/Image" ./AnyKernel3/ || error "Failed to copy Image" cd AnyKernel3 || error "Failed to enter AnyKernel3 directory" zip -r "AnyKernel3_${KSU_VERSION}_${DEVICE_NAME}_SuKiSu.zip" ./* || error "Packaging failed"

OUTPUT_DIR="${GITHUB_WORKSPACE:-$PWD}/output" mkdir -p "$OUTPUT_DIR" || error "Failed to create output directory" cp "$WORKSPACE/AnyKernel3/AnyKernel3_${KSU_VERSION}_${DEVICE_NAME}_SuKiSu.zip" "$OUTPUT_DIR/" cp "$KERNEL_WORKSPACE/kernel_platform/common/out/arch/arm64/boot/Image" "$OUTPUT_DIR/"

info "Kernel package path: $OUTPUT_DIR/AnyKernel3_${KSU_VERSION}_${DEVICE_NAME}_SuKiSu.zip" info "Image path: $OUTPUT_DIR/Image" info "Build complete. Artifacts are in $OUTPUT_DIR"
