...
# Device selection
info "Please select the device to build:"
...
# Prompt for kernel version suffix
read -p "Enter kernel version suffix (can include Chinese and emoji, press Enter for default: -android14-TG@qdykernel): " input_suffix
...
read -p "Enable kpm? (press Enter to enable by default) [y/N]: " kpm
[[ "$kpm" =~ [yY] ]] && ENABLE_KPM=true

read -p "Enable lz4+zstd? (press Enter to enable by default) [y/N]: " lz4
...
for f in kernel_platform/{common,msm-kernel,external/dtc}/scripts/setlocalversion; do
    # Replace the last echo "$res" with echo "<your suffix>"
...
# Create output directory on C drive (via WSL)
WIN_OUTPUT_DIR="/mnt/c/Kernel_Build/${DEVICE_NAME}/"
mkdir -p "$WIN_OUTPUT_DIR" || error "Failed to create Windows directory, C drive may not be mounted. Files will be saved in Linux directory: $WORKSPACE/AnyKernel3/AnyKernel3_${KSU_VERSION}_${DEVICE_NAME}_SuKiSu.zip"
...
info "Please check the kernel package and Image file in the C drive directory."
info "Cleaning all build files..."
sudo rm -rf "$WORKSPACE" || error "Failed to delete workspace, it may not have been created"
info "Cleanup complete! Running the script next time will re-pull the source and build the kernel again."
